...
from meridian_partition.documents import elements, ontology
from meridian_partition.documents.html_sanitization import sanitize_attributes
from meridian_partition.documents.mappings import (
...
from meridian_partition.partition.common.metadata import (
    HEADING_TAGS,
...
def ontology_to_meridian_partition_elements(
    ...
    """
    Converts an OntologyElement object to a list of meridian_partition Element objects.
    ...
    is injected to meridian_partition Element in ElementMetadata.text_as_html attribute.

    For Layout elements, the function creates an empty Text Element (with the
    HTML code injected the same way).

    TODO (Pluto): Better way would be to have special Element type in MeridianPartition
    ...
    Returns:
        list[Element]: A list of meridian_partition Element objects.
    ...
    elements_with_depth = _ontology_to_meridian_partition_elements(
        ontology_element,
        parent_id=parent_id,
...
def _ontology_to_meridian_partition_elements(
    ontology_element: ontology.OntologyElement,
    parent_id: str | None = None,
    page_number: int | None = None,
    depth: int = 0,
    ...
    """Recursive worker for `ontology_to_meridian_partition_elements`.
    ...
    if ontology_element.elementType == ontology.ElementTypeEnum.layout and depth <= RECURSION_LIMIT:
        ...
        for child in ontology_element.children:
            child = _ontology_to_meridian_partition_elements(
                child,
        ...
        element_class: type[elements.Element] = ONTOLOGY_CLASS_TO_MERIDIAN_PARTITION_ELEMENT_TYPE[
        ...
        meridian_partition_element = element_class(
        ...
        elements_to_return = [(meridian_partition_element, depth)]
...
def can_meridian_partition_elements_be_merged(
...
def meridian_partition_elements_to_ontology(
    meridian_partition_elements: Sequence[elements.Element],
) -> ontology.OntologyElement:
    """
    Converts a sequence of meridian_partition Element objects to an OntologyElement object.
    ...
    """
    if not meridian_partition_elements:
    ...
    root_element_id = meridian_partition_elements[0].metadata.parent_id

    if root_element_id is None:
        root_element_id = ontology.OntologyElement.generate_unique_id()
        meridian_partition_elements[0].metadata.parent_id = root_element_id
    ...
    for element in meridian_partition_elements:
...
def parse_html_to_ontology_element(soup: Tag, recursion_depth: int = 1) -> ontology.OntologyElement:
    ...
    First tries to recognize a class from MeridianPartition Ontology, then if class is matched tries
...
def extract_tag_and_ontology_class_from_tag(
    ...
    the HTML tag. If not recognized soup.name and MeridianPartitionText is returned.
...
def get_sanitized_attributes(soup: Tag, tag_name: str | None = None) -> dict[str, str | list[str]]:
    ...
    ``meridian_partition.documents.html_sanitization``). Escaping here as well would

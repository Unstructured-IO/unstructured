...
from meridian_partition.cleaners.core import remove_punctuation
from meridian_partition.logger import trace_logger
from meridian_partition.nlp.english_words import ENGLISH_WORDS
from meridian_partition.nlp.patterns import (
...
from meridian_partition.nlp.tokenize import pos_tag, sent_tokenize, word_tokenize
...
def is_possible_narrative_text(
    ...
    """
    _language_checks = os.environ.get("MERIDIAN_PARTITION_LANGUAGE_CHECKS")
    ...
    # cast it to a float
    cap_threshold = float(
        os.environ.get("MERIDIAN_PARTITION_NARRATIVE_TEXT_CAP_THRESHOLD", cap_threshold),
    )
    ...
    non_alpha_threshold = float(
        os.environ.get("MERIDIAN_PARTITION_NARRATIVE_TEXT_NON_ALPHA_THRESHOLD", non_alpha_threshold),
    )
...
def is_possible_title(
    ...
    title_max_word_length = int(
        os.environ.get("MERIDIAN_PARTITION_TITLE_MAX_WORD_LENGTH", title_max_word_length),
    )
    ...
    non_alpha_threshold = float(
        os.environ.get("MERIDIAN_PARTITION_TITLE_NON_ALPHA_THRESHOLD", non_alpha_threshold),
    )
    if under_non_alpha_ratio(text, threshold=non_alpha_threshold):
        return False

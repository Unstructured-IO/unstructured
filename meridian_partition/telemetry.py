...
from meridian_partition.utils import _telemetry_opt_out, scarf_analytics

_P = ParamSpec("_P")
_R = TypeVar("_R")

_ENDPOINT = "https://packages.meridian_partition.io/python-telemetry"
...
def _schedule_delivery(build_params: Callable[[], dict[str, Any]]) -> None:
    ...
    try:
        ...
        threading.Thread(
            target=_deliver,
            args=(params,),
            name="meridian_partition-telemetry",
            daemon=True,
        ).start()
    except Exception:

...
from meridian_partition.__version__ import __version__
...
def _pip_hint_for_file_type(file_type: FileType) -> str:
    ...
    if extra:
        return f'pip install "meridian_partition[{extra}]"'
    return "pip install meridian_partition"
    ...
    if file_type.extra_name == "audio":
        if not _audio_extra_import_ok():
            ...
            messages.append(
                ...
                'Install with: pip install "meridian_partition[audio]"',
            )
...
def environment_rows() -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        ("meridian_partition", "ok", __version__),
    ]
...
def build_report() -> str:
    """Full diagnostic report for :command:`meridian_partition doctor` with no filter."""
    parts: list[str] = []

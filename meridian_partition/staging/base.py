...
from meridian_partition.file_utils.ndjson import dumps as ndjson_dumps
from meridian_partition.partition.common.common import exactly_one
from meridian_partition.utils import Point, dependency_exists, requires_dependencies
...
def create_file_from_elements(
    ...
    *,
    ...
    Example:
        ...
        >>> from meridian_partition.staging.base import create_file_from_elements
...
def convert_to_coco(
    ...
    contributors: tuple[str] = ("MeridianPartition Developers",),
) -> dict[str, Any]:
    from meridian_partition.documents.elements import TYPE_TO_TEXT_ELEMENT_MAP
    ...
    coco_dataset["info"] = {
        "description": (
            ...
            else f"MeridianPartition COCO Dataset {datetime.now().strftime('%Y-%m-%d')}"
        ),

...
OUTPUT_FOLDER_NAME=embed-nvidia
...
if [ -z "$NVIDIA_API_KEY" ]; then
  echo "Skipping local nvidia ingest test because the NVIDIA_API_KEY env var is not set."
  exit 8
fi
...
PYTHONPATH=${PYTHONPATH:-.} "$RUN_SCRIPT" \
  ...
  --work-dir "$WORK_DIR" \
  --embedding-provider "langchain-nvidia-ai-endpoints" \

...
from unstructured.partition.html.partition import (
...
def test_nested_text_tags():
    ...
    assert elements == [NarrativeText("There is some text here.")]
...
def test_partition_html_from_filename_pulls_last_modified_from_filesystem(tmp_path: pathlib.Path):
    modified_timestamp = dt.datetime(
        year=2024, month=6, day=16, hour=12, minute=13, second=29
    ).timestamp()
    file_path = tmp_path / "document.html"
    file_path.write_text("<p>Time is an illusion. Lunchtime doubly so.")
    os.utime(file_path, (modified_timestamp, modified_timestamp))

    (element,) = partition_html(str(file_path))

    assert isinstance(element, NarrativeText)
    assert element.metadata.last_modified == "2024-06-16T12:13:29"
...
def test_partition_html_produces_one_cell_for_each_original_table_cell():
    html_text = (
    ...
    assert element == Table("foo bar")
    assert element.metadata.text_as_html == "<table><tr><td>foo</td><td>bar</td></tr></table>"

...
from meridian_partition.metrics.evaluate import (
...
MERIDIAN_PARTITION_OUTPUT_DIRNAME = "unstructured_output"
...
MERIDIAN_PARTITION_CCT_DIRNAME = "unstructured_output_cct"
MERIDIAN_PARTITION_TABLE_STRUCTURE_DIRNAME = "unstructured_output_table_structure"
...
def mock_dependencies():
    with (
        patch("meridian_partition.metrics.evaluate.calculate_accuracy") as mock_calculate_accuracy,
        patch(
            "meridian_partition.metrics.evaluate.calculate_percent_missing_text"
        ) as mock_calculate_percent_missing_text,
        patch.object(TextExtractionMetricsCalculator, "_get_ccts") as mock_get_ccts,
        patch(
            "meridian_partition.metrics.evaluate.get_element_type_frequency"
        ) as mock_get_element_type_frequency,
        patch(
            "meridian_partition.metrics.evaluate.calculate_element_type_percent_match"
        ) as mock_calculate_element_type_percent_match,
        patch("meridian_partition.metrics.evaluate._read_text_file") as mock_read_text_file,
        patch.object(Path, "exists") as mock_path_exists,
        patch(
            "meridian_partition.metrics.evaluate.TableEvalProcessor.from_json_files"
...
def test_text_extraction_evaluation():
    output_dir = os.path.join(TESTING_FILE_DIR, MERIDIAN_PARTITION_OUTPUT_DIRNAME)
...
@pytest.mark.parametrize(
    ...
    [
        (
            TextExtractionMetricsCalculator,
            MERIDIAN_PARTITION_CCT_DIRNAME,
            GOLD_CCT_DIRNAME,
        ...
        (
            TableStructureMetricsCalculator,
            MERIDIAN_PARTITION_TABLE_STRUCTURE_DIRNAME,
        ...
        (
            ElementTypeMetricsCalculator,
            MERIDIAN_PARTITION_OUTPUT_DIRNAME,
...
def test_text_extraction_evaluation_type_txt():
    output_dir = os.path.join(TESTING_FILE_DIR, MERIDIAN_PARTITION_CCT_DIRNAME)
    source_dir = os.path.join(TESTING_FILE_DIR, GOLD_CCT_DIRNAME)
    export_dir = os.path.join(TESTING_FILE_DIR, "test_evaluate_results_cct")
...
def test_table_structure_evaluation():
    output_dir = os.path.join(TESTING_FILE_DIR, MERIDIAN_PARTITION_TABLE_STRUCTURE_DIRNAME)

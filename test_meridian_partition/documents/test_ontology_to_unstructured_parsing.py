...
from meridian_partition.documents.elements import ElementMetadata, Text
from meridian_partition.documents.ontology import (
    Column,
    Document,
    Heading,
    Hyperlink,
...
from meridian_partition.embed.openai import OpenAIEmbeddingConfig, OpenAIEmbeddingEncoder
...
def test_page_number_is_passed_correctly():
    ...
    meridian_partition_elements = ontology_to_meridian_partition_elements(ontology)
    page1, p1, page2, p2 = meridian_partition_elements
...
def test_invalid_page_number_is_not_passed():
    ...
    meridian_partition_elements = ontology_to_meridian_partition_elements(ontology)
    page1, p1 = meridian_partition_elements
...
def test_category_depth_is_not_derived_from_layout_nesting():
    ...
    meridian_partition_elements = ontology_to_meridian_partition_elements(ontology)
    page1, p1, page2, c1, p2, c2, p3 = meridian_partition_elements
...
def test_category_depth_is_derived_from_heading_level():
    ...
    meridian_partition_elements = ontology_to_meridian_partition_elements(ontology)
    _page, title, subtitle, heading, body = meridian_partition_elements
...
def test_partition_html_v2_assigns_heading_based_parent_id():
    ...
    callers (meridian_partition and the VLM partitioner) go through `partition_html`, so the
...
def test_converter_leaves_content_parent_id_for_the_metadata_layer():
    ...
    page, section_a, intro, sub_a1 = ontology_to_meridian_partition_elements(ontology)
...
def test_chunking_is_applied_on_elements():
    ...
    chunked_basic = chunk_elements(meridian_partition_elements)
    assert str(chunked_basic[0]) == "Paragraph1\n\nParagraph2\n\nParagraph3"
    chunked_by_title = chunk_by_title(meridian_partition_elements)
...
def test_embeddings_are_applied_on_elements(mocker):
    ...
    elements = encoder.embed_documents(
        elements=meridian_partition_elements,
    )
...
def test_ingest(html_file_path, json_file_path):
    ...
    meridian_partition_elements = ontology_to_meridian_partition_elements(ontology)
    assert meridian_partition_elements == expected_json_elements


@pytest.mark.parametrize("json_file_path", ["meridian_partition_json_output/example.json"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("html_files/three_tables.html", "meridian_partition_json_output/three_tables.json"),
        (
            "html_files/example_with_inline_fields.html",
            "meridian_partition_json_output/example_with_inline_fields.json",
        ),
    ],
)
...
def test_inline_elements_are_squeezed():
    ...
    meridian_partition_elements = ontology_to_meridian_partition_elements(ontology)
    assert len(meridian_partition_elements) == 2

    page, text1 = meridian_partition_elements
...
def test_inline_elements_are_squeezed_when_image():
    ...
    meridian_partition_elements = ontology_to_meridian_partition_elements(ontology)
    assert len(meridian_partition_elements) == 4

    page, text1, image, text2 = meridian_partition_elements
...
def test_inline_elements_are_squeezed_when_table():
    ...
    page, text1, table1, text3 = meridian_partition_elements
...
def test_inline_elements_are_on_many_depths():
    ...
    assert len(meridian_partition_elements) == 6

    page, text1, section1, section2, text2, text3 = meridian_partition_elements
...
def test_combine_inline_elements_merges_a_long_run_without_reparsing(monkeypatch):
    ...
    import meridian_partition.partition.html.transformations as tr

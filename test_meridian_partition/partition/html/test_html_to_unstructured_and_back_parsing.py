...
from meridian_partition.documents.ontology import Address, Paragraph
...
def _parse_to_meridian_partition_elements_and_back_to_html(html_as_str: str):
    meridian_partition_elements = partition_html(
        text=html_as_str, add_img_alt_text=False, html_parser_version="v2", unique_element_ids=True
    )
    parsed_ontology = meridian_partition_elements_to_ontology(meridian_partition_elements)
    return meridian_partition_elements, parsed_ontology
...
def test_simple_narrative_text_with_id():
    ...
    _assert_elements_equal(meridian_partition_elements, expected_elements)
...
def test_inline_elements_are_squeezed_when_text_wrapped_into_paragraphs():
    ...
    # We would like it to be parsed to MeridianPartitionElements as [Page, NarrativeText]
    ...
    assert isinstance(meridian_partition_elements[0], Text)
    assert isinstance(meridian_partition_elements[1], NarrativeText)
...
def test_alternate_text_from_image_is_passed():
    ...
    meridian_partition_elements = ontology_to_meridian_partition_elements(page)
    assert len(meridian_partition_elements) == 2
    assert "ALT TEXT Logo" in meridian_partition_elements[1].text

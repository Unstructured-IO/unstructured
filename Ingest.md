...
to be checked into CI under test_unstructured_ingest/expected-structured-output/<folder-name-relevant-to-your-dataset>. So, you'll need to `git add` those so that `test-ingest.sh` passes in CI.
...
- [ ] Add a script test_unstructured_ingest/test-ingest-<the-new-data-source>.sh. It's json output files should be no more than 100K total.

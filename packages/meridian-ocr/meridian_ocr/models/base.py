...
from meridian_ocr.models.meridian_ocr_model import MeridianOCRModel
from meridian_ocr.models.yolox import MODEL_TYPES as YOLOX_MODEL_TYPES
from meridian_ocr.models.yolox import MeridianOCRYoloXModel
from meridian_ocr.utils import LazyDict
...
class Models(object):
        ...
        if cls._instance is None:
            with cls._lock:
                if cls._instance is None:
                    ...
                    cls.models: Dict[str, MeridianOCRModel] = {}
    ...
    def __setitem__(self, key: str, value: MeridianOCRModel):
...
def get_default_model_mappings() -> Tuple[
    Dict[str, Type[MeridianOCRModel]],
    Dict[str, dict | LazyDict],
]:
    """default model mappings for models that are in `meridian_ocr` repo"""
    return {
        **dict.fromkeys(DETECTRON2_ONNX_MODEL_TYPES, MeridianOCRDetectronONNXModel),
        **dict.fromkeys(YOLOX_MODEL_TYPES, MeridianOCRYoloXModel),
...
def register_new_model(model_config: dict, model_class: MeridianOCRModel):
...
def get_model(model_name: Optional[str] = None) -> MeridianOCRModel:
    ...
    if model_name is None:
        default_name_from_env = os.environ.get("MERIDIAN_OCR_DEFAULT_MODEL_NAME")
    ...
    with model_lock:
        ...
        model: MeridianOCRModel = model_class_map[model_name]()

...
class MockModel(MeridianOCRObjectDetectionModel):
    call_count = 0
...
def test_get_model_with_lazydict_config(monkeypatch):
    ...
    """
    from meridian_ocr.utils import LazyDict, LazyEvaluateInfo
...
def test_raises_uninitialized():
    with pytest.raises(ModelNotInitializedError):
        models.MeridianOCRDetectronONNXModel().predict(None)
...
def test_deduplicate_detected_elements():
    ...
    from meridian_ocr.inference.elements import intersections
...
def test_clean_type():
    from meridian_ocr.inference.layout import LayoutElement
    from meridian_ocr.models.base import get_model
...
def test_env_variables_override_default_model(monkeypatch):
    ...
    with (
        mock.patch.dict(
            models.os.environ,
            {"MERIDIAN_OCR_DEFAULT_MODEL_NAME": "yolox"},
        ),
        mock.patch.dict(models.model_class_map, {"yolox": MockModel}),
    ):
...
def test_env_variables_override_initialization_params(monkeypatch):
    ...
    with (
        mock.patch.dict(
            models.os.environ,
            {"MERIDIAN_OCR_DEFAULT_MODEL_INITIALIZE_PARAMS_JSON_PATH": "fake_json.json"},
        ),

...
import meridian_ocr.models.detectron2onnx as detectron2
...
def test_load_model(model_path, label_map):
    with patch.object(detectron2.onnxruntime, "InferenceSession", return_value=True):
        model = detectron2.MeridianOCRDetectronONNXModel()
...
def test_meridian_ocr_detectron_model():
    model = detectron2.MeridianOCRDetectronONNXModel()
    model.model = 1
    with patch.object(detectron2.MeridianOCRDetectronONNXModel, "predict", return_value=[]):
        result = model(None)

...
import meridian_ocr.models.base as models
from meridian_ocr.constants import IsExtracted
from meridian_ocr.inference import elements, layout, layoutelement, pdf_image
from meridian_ocr.inference.elements import (
    EmbeddedTextRegion,
    ImageTextRegion,
)
from meridian_ocr.models.meridian_ocr_model import (
    MeridianOCRElementExtractionModel,
...
def test_process_data_with_model(monkeypatch, mock_final_layout, model_name):
    ...
    with (
        ...
        patch(
            "meridian_ocr.inference.layout.MeridianOCRObjectDetectionModel",
            MockLayoutModel,
        ),
        open("") as fp,
    ):
...
def test_process_file_with_model(monkeypatch, mock_final_layout, model_name):
    ...
    monkeypatch.setattr(models.MeridianOCRDetectronONNXModel, "initialize", mock_initialize)
    filename = ""
...
def test_page_numbers_in_page_objects():
    with patch(
        "meridian_ocr.inference.layout.PageLayout.get_elements_with_detection_model",
...
class MockDetectionModel(layout.MeridianOCRObjectDetectionModel):
    def initialize(self, *args, **kwargs):
        pass
...
class MockMeridianOCRElementExtractionModel(MeridianOCRElementExtractionModel):
    def initialize(self, *args, **kwargs):
...
class MockMeridianOCRDetectionModel(MeridianOCRObjectDetectionModel):
    def initialize(self, *args, **kwargs):
        return super().initialize(*args, **kwargs)
...
@pytest.mark.parametrize(
    ...
    [
        (MockMeridianOCRElementExtractionModel, False),
        (MockMeridianOCRDetectionModel, True),
    ],
)

...
for your use case. Docker targets live in the repository-root `Makefile`:
...
Then install all dependencies (base, extras, dev, test, and lint groups) from the repository root:

```bash
make install
```

This runs `uv sync --locked --all-packages --all-extras --all-groups`, which creates a single virtual
environment at the repository root for every package in the workspace and installs everything in one step.
No need to manually create or activate a virtualenv.

The commands below are run from `packages/meridian-partition`. To install only specific document-type extras:
...
Encountered a bug? Please create a new [GitHub issue](https://github.com/Unstructured-IO/unstructured/issues/new/choose) and use our bug report template to describe the problem. To help us diagnose the issue, use the `python packages/meridian-partition/scripts/collect_env.py` command to gather your system's environment information and include it in your report. Your assistance helps us continuously improve our software - thank you!

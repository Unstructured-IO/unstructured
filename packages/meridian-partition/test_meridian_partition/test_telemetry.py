...
This module sets an opt-out before importing meridian_partition so the package initializer cannot
...
# Set an opt-out before importing meridian_partition so package initialization is hermetic.
...
from meridian_partition import utils
...
def telemetry_mocks(monkeypatch):
    ...
    mock_get = Mock()
    mock_run = Mock()
    monkeypatch.setattr("meridian_partition.utils.requests.get", mock_get)
    monkeypatch.setattr("meridian_partition.utils.subprocess.run", mock_run)
...
class DescribeScarfAnalytics:
    ...
    def it_sends_telemetry_by_default(self, telemetry_mocks):
        ...
        assert call_args[0][0] == "https://packages.meridian_partition.io/python-telemetry"
    ...
    def it_sends_telemetry_with_correct_dev_param(
        ...
        mock_get, mock_run = telemetry_mocks
        monkeypatch.setattr("meridian_partition.utils.__version__", version_val)
    ...
    def it_suppresses_requests_exceptions(self, telemetry_mocks):
        ...
        mock_get.assert_called_once()
        mock_run.assert_called_once()
        assert mock_get.call_args[0][0] == "https://packages.meridian_partition.io/python-telemetry"
    ...
    def it_import_meridian_partition_succeeds_when_opted_out(self):
        ...
        result = subprocess.run(
            [sys.executable, "-c", "import meridian_partition; print('ok')"],
            env=env,
            cwd=project_root,
            capture_output=True,
            text=True,
    ...
    def it_import_meridian_partition_runs_telemetry_once_by_default(self):
...
"""
    ...
    def it_import_meridian_partition_uses_a_bounded_gpu_probe(self):
...
assert m_get.call_count == 1
assert m_get.call_args.args[0] == "https://packages.meridian_partition.io/python-telemetry"

...
from meridian_partition.documents.ontology import OntologyElement
...
def test_ontology_to_meridian_partition_mapping_has_valid_types():
    for (
        ontology_element,
        meridian_partition_element,
    ) in ONTOLOGY_CLASS_TO_MERIDIAN_PARTITION_ELEMENT_TYPE.items():
        assert issubclass(meridian_partition_element, elements.Element)
        assert issubclass(ontology_element, ontology.OntologyElement)


def test_all_ontology_elements_are_defined_in_mapping_to_meridian_partition():
    for ontology_element in get_all_subclasses(ontology.OntologyElement):
        assert ontology_element in ONTOLOGY_CLASS_TO_MERIDIAN_PARTITION_ELEMENT_TYPE

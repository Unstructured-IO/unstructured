"""Test suite for `meridian_partition.partition.email` module."""
...
def test_partition_email_silently_skips_attachments_it_cannot_partition():
    ...
    with patch("meridian_partition.partition.auto.partition") as mock_partition:
...
class DescribeEmailPartitionerOptions:
    """Unit-test suite for `meridian_partition.partition.email.EmailPartitioningContext` objects."""
    ...
    def get_last_modified_date_(self, request: FixtureRequest) -> Mock:
        return function_mock(request, "meridian_partition.partition.email.get_last_modified_date")

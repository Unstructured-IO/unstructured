...
from meridian_partition.partition import text_type
from test_meridian_partition.nlp.mock_nlp import (
    mock_pos_tag,
    mock_sent_tokenize,
    mock_word_tokenize,
)
...
def test_is_possible_narrative_text(text, expected, monkeypatch):
    ...
    monkeypatch.setenv("MERIDIAN_PARTITION_LANGUAGE_CHECKS", "true")
...
def test_set_caps_ratio_with_environment_variable(monkeypatch):
    monkeypatch.setattr(text_type, "word_tokenize", mock_word_tokenize)
    monkeypatch.setattr(text_type, "sent_tokenize", mock_sent_tokenize)
    monkeypatch.setenv("MERIDIAN_PARTITION_NARRATIVE_TEXT_CAP_THRESHOLD", str(0.8))
...
def test_set_title_non_alpha_threshold_with_environment_variable(monkeypatch):
    monkeypatch.setattr(text_type, "word_tokenize", mock_word_tokenize)
    monkeypatch.setattr(text_type, "sent_tokenize", mock_sent_tokenize)
    monkeypatch.setenv("MERIDIAN_PARTITION_TITLE_NON_ALPHA_THRESHOLD", str(0.8))
...
def test_set_narrative_text_non_alpha_threshold_with_environment_variable(monkeypatch):
    monkeypatch.setattr(text_type, "word_tokenize", mock_word_tokenize)
    monkeypatch.setattr(text_type, "sent_tokenize", mock_sent_tokenize)
    monkeypatch.setenv("MERIDIAN_PARTITION_NARRATIVE_TEXT_NON_ALPHA_THRESHOLD", str(0.8))
...
def test_set_title_max_word_length_with_environment_variable(monkeypatch):
    monkeypatch.setattr(text_type, "word_tokenize", mock_word_tokenize)
    monkeypatch.setattr(text_type, "sent_tokenize", mock_sent_tokenize)
    monkeypatch.setenv("MERIDIAN_PARTITION_TITLE_MAX_WORD_LENGTH", str(5))

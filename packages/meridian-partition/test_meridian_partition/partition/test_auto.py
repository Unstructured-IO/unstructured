...
from meridian_partition.file_utils.filetype import detect_filetype
from meridian_partition.file_utils.model import FileType, create_file_type, register_partitioner
from meridian_partition.partition.auto import _PartitionerLoader, partition
...
def test_partition_forwards_strategy_arg_to_partition_docx_and_its_brokers(
    ...
    """
    from meridian_partition.partition.docx import _DocxPartitioner
...
def test_partition_md_from_url_works_with_embedded_html():
    ...
    assert "meridian_partition" in elements[1].text
...
def test_auto_partition_pdf_with_fast_strategy(request: FixtureRequest):
    partition_pdf_ = function_mock(
        request,
        "meridian_partition.partition.pdf.partition_pdf",
...
def test_auto_handles_kwarg_with_infer_table_structure(infer_bool):
    with patch(
        "meridian_partition.partition.pdf_image.ocr.process_file_with_ocr",
...
def test_partition_forwards_strategy_arg_to_partition_pptx_and_its_brokers(
    ...
    """
    from meridian_partition.partition.pptx import _PptxPartitioner
...
def test_auto_partition_raises_with_bad_type(request: FixtureRequest):
    detect_filetype_ = function_mock(
        request, "meridian_partition.partition.auto.detect_filetype", return_value=FileType.UNK
    )
...
def test_auto_partition_from_url_routes_timeout_to_HTTP_request(request: FixtureRequest):
    file_and_type_from_url_ = function_mock(
        request,
        "meridian_partition.partition.auto.file_and_type_from_url",
...
def test_auto_partition_image_formats_languages_for_tesseract(request: FixtureRequest):
    process_file_with_ocr_ = function_mock(
        request, "meridian_partition.partition.pdf_image.ocr.process_file_with_ocr"
    )
...
def test_auto_partition_adds_filetype_to_metadata(
    ...
    partition_fn_ = function_mock(
        request,
        f"meridian_partition.partition.{shortname}.partition_{shortname}",
...
def test_detect_filetype_maps_file_to_bytes_io_when_spooled_temp_file_used(mocker):
    ...
    mocker.patch("meridian_partition.file_utils.filetype._FileTypeDetector", detect_filetype_mock)
...
def test_auto_partition_that_requires_extras_raises_when_dependencies_are_not_installed(
    ...
    dependency_exists_ = function_mock(
        request, "meridian_partition.partition.auto.dependency_exists", return_value=False
    )

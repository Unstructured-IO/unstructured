...
"""Test suite for `meridian_partition.partition.pptx` module."""
...
from meridian_partition.partition.pptx import (
...
def test_partition_pptx_level_detection(tmp_path: pathlib.Path):
    ...
    # NOTE(newelh) - python_pptx does not create full bullet xml, so meridian_partition will
...
class DescribePptxPartitionerOptions:
    """Unit-test suite for `meridian_partition.partition.xlsx.PptxPartitionerOptions` objects."""
    ...
    def get_last_modified_date_(self, request: FixtureRequest):
        return function_mock(request, "meridian_partition.partition.pptx.get_last_modified_date")

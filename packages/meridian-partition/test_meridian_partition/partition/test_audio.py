...
from meridian_partition.documents.elements import NarrativeText
from meridian_partition.file_utils.model import FileType
from meridian_partition.partition.audio import partition_audio
...
def mock_stt_agent() -> Generator[MagicMock, None, None]:
    ...
    ``@patch("meridian_partition.partition.audio.SpeechToTextAgent.get_agent")`` directly so
    the mock is visible in the function signature.
    """
    with patch("meridian_partition.partition.audio.SpeechToTextAgent.get_agent") as mock_get_agent:
...
def _clear_stt_cache() -> Generator[None, None, None]:
    ...
    """
    from meridian_partition.partition.utils.speech_to_text.speech_to_text_interface import (
        SpeechToTextAgent,
    )
...
@patch("meridian_partition.partition.audio.SpeechToTextAgent.get_agent")
...
@patch("meridian_partition.partition.audio.SpeechToTextAgent.get_agent")
@patch("meridian_partition.partition.audio.detect_filetype")
...
def test_audio_suffix(metadata_filename, file_name, expected):
    ...
    from meridian_partition.partition.audio import _audio_suffix
...
class TestSpeechToTextAgentInterface:
    ...
    def test_get_agent_uses_env_config_when_no_module_given(self):
        ...
        sentinel = "meridian_partition.partition.utils.speech_to_text.whisper_stt.SentinelAgent"
        with (
    ...
    def test_get_instance_rejects_whitelisted_module_attribute_that_is_not_a_class(self):
        ...
        agent_module = "meridian_partition.partition.utils.speech_to_text.whisper_stt.some_function"
        mock_mod = MagicMock()
        mock_mod.some_function = lambda x: x  # not a class

        with patch(
            "meridian_partition.partition.utils.speech_to_text.speech_to_text_interface.importlib.import_module",
    ...
    def test_get_instance_rejects_class_that_does_not_subclass_speech_to_text_agent(self):
        ...
        agent_module = "meridian_partition.partition.utils.speech_to_text.whisper_stt.WrongClass"
        mock_mod = MagicMock()

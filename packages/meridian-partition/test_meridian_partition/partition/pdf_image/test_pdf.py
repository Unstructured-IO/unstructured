...
from meridian_partition.partition.pdf_image import ocr, pdfminer_processing
...
@mock.patch.dict(os.environ, {"MERIDIAN_PARTITION_HI_RES_MODEL_NAME": "checkbox"})
...
def test_partition_pdf_metadata_date(
    ...
    mocker.patch(
        "meridian_partition.partition.pdf.get_last_modified_date",
...
def test_default_hi_res_model(env, expected, monkeypatch):
    if env is not None:
        monkeypatch.setenv("MERIDIAN_PARTITION_HI_RES_MODEL_NAME", env)

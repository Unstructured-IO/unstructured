...
"""Test suite for `meridian_partition.file_utils.filetype`."""
...
def test_it_falls_back_to_filetype_mime_guessing_when_libmagic_returns_unrecognized_mime_type(
    ...
    with patch("meridian_partition.file_utils.filetype.LIBMAGIC_AVAILABLE", True):
        with patch(
            "meridian_partition.file_utils.filetype.ft.guess_mime", return_value=fallback_mime_type
        ):
            with open(example_doc_path(file_name), "rb") as f:
                file = io.BytesIO(f.read())
...
def it_routes_not_meridian_partition_payload_json_away_from_ndjson_via_detect_filetype():
...
def LIBMAGIC_AVAILABLE_False():
    with patch("meridian_partition.file_utils.filetype.LIBMAGIC_AVAILABLE", False) as m:
        yield m
...
class Describe_FileTypeDetectionContext:
    """Unit-test suite for `meridian_partition.file_utils.filetype._FileTypeDetectionContext`."""
    ...
    def and_it_provides_the_MIME_type_from_path_using_filetype_lib_when_magic_is_unavailable(self):
        with patch("meridian_partition.file_utils.filetype.LIBMAGIC_AVAILABLE", False):
...
class Describe_OleFileDetector:
    """Unit-test suite for `meridian_partition.file_utils.filetype._OleFileDetector`."""
...
class Describe_TextFileDifferentiator:
    """Unit-test suite for `meridian_partition.file_utils.filetype._TextFileDifferentiator`."""
...
class Describe_ZipFileDetector:
    """Unit-test suite for `meridian_partition.file_utils.filetype._ZipFileDetector`."""

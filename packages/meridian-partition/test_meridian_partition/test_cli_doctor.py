"""Tests for :mod:`meridian_partition.cli` and :mod:`meridian_partition.doctor`."""
...
from meridian_partition.cli import _cmd_doctor, main
from meridian_partition.doctor import (
    _libmagic_status,
...
def test_evaluate_specifier_dedupes_image_family() -> None:
    with mock.patch("meridian_partition.doctor.evaluate_file_type_capability") as ev:
...
def test_evaluate_specifier_multiple_targets() -> None:
    ...
    with (
        mock.patch(
            "meridian_partition.doctor.resolve_specifier",
            return_value=[FileType.PDF, FileType.CSV],
        ),
        mock.patch(
            "meridian_partition.doctor.evaluate_file_type_capability",
...
def test_evaluate_specifier_dedupes_repeated_message_lines() -> None:
    with (
        mock.patch("meridian_partition.doctor.resolve_specifier", return_value=[FileType.PDF]),
        mock.patch(
            "meridian_partition.doctor.evaluate_file_type_capability",
...
def test_main_doctor_invocation() -> None:
    with mock.patch("meridian_partition.cli._cmd_doctor", return_value=0) as m:
        assert main(["doctor"]) == 0
        m.assert_called_once_with([])


def test_main_no_argv_reads_sys_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("sys.argv", ["meridian_partition", "doctor", "--for", "txt"])
    with mock.patch("meridian_partition.cli._cmd_doctor", return_value=0) as m:
        assert main(None) == 0
...
def test_cmd_doctor_for_missing_dep_exit_code() -> None:
    with mock.patch(
        "meridian_partition.doctor.evaluate_specifier",
...
def test_cmd_doctor_file_prints_messages(
    ...
    with mock.patch(
        "meridian_partition.doctor.file_path_to_capability",
...
def test_evaluate_doc_requires_soffice_when_deps_ok() -> None:
    with mock.patch("meridian_partition.doctor.python_import_deps_ok", return_value=(True, [])):
        with mock.patch("meridian_partition.doctor._libreoffice_on_path", return_value=None):
...
def test_evaluate_epub_requires_pandoc_binary_when_python_ok(
    ...
    with mock.patch("meridian_partition.doctor.python_import_deps_ok", return_value=(True, [])):
        ...
        monkeypatch.setattr("meridian_partition.doctor.shutil.which", _which)
...
def test_evaluate_audio_not_ready_without_ffmpeg(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("meridian_partition.doctor.shutil.which", _which)
    with mock.patch("meridian_partition.doctor._audio_extra_import_ok", return_value=True):
...
def test_environment_rows() -> None:
    ...
    assert "meridian_partition" in labels
...
def test_pip_hint_for_file_type() -> None:
    ...
    assert _pip_hint_for_file_type(FileType.HTML) == "pip install meridian_partition"

"""Command-line interface for :mod:`meridian_partition`."""
...
def _cmd_doctor(argv: list[str] | None = None) -> int:
    from meridian_partition.doctor import build_report, evaluate_specifier, file_path_to_capability

    parser = argparse.ArgumentParser(
        prog="meridian_partition doctor",
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for the ``meridian_partition`` console script."""
    argv = list(sys.argv[1:] if argv is None else argv)

    if not argv or argv[0] in ("-h", "--help"):
        parser = argparse.ArgumentParser(
            prog="meridian_partition",
            description="MeridianPartition document processing utilities.",
        )
        parser.add_argument(
            "command",
            nargs="?",
    ...
    print(
        f"meridian_partition: unknown command {argv[0]!r}. Try `meridian_partition --help`.",
        file=sys.stderr,
    )
    return 2

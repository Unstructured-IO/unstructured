...
HTML Tags <-> Elements Ontology <-> MeridianPartition Element classes
...
def get_ontology_to_meridian_partition_type_mapping() -> dict[
    Type[ontology.OntologyElement], Type[Element]
]:
    """
    Get a mapping of ontology element to meridian_partition type.
    ...
    ```
        ontology_elements_list = json.loads(
            Path("meridian_partition_element_ontology.json").read_text()
        )
        ontology_to_meridian_partition_class_mapping = {
    ...
    return ontology_to_meridian_partition_class_mapping

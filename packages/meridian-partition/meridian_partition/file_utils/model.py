...
class FileType(enum.Enum):
    """The collection of file-types recognized by `meridian_partition`.
    ...
    def extra_name(self) -> str | None:
        ...
        Like "image" for PNG, as in `pip install "meridian_partition[image]"`.

        `None` when partitioning this file-type requires only the base `meridian_partition` install.
        """
    ...
    def importable_package_dependencies(self) -> tuple[str, ...]:
        ...
        like `pip install "meridian_partition[docx]"` loads the `python-docx` package.
    ...
    def partitioner_module_qname(self) -> str:
        ...
        e.g. "meridian_partition.partition.docx" for FileType.DOCX.
        """
        # -- Raise when this property is accessed on a FileType member that has no partitioner
        # -- shortname. This prevents a harder-to-find bug from appearing far away from this call
        ...
        return f"meridian_partition.partition.{shortname}"

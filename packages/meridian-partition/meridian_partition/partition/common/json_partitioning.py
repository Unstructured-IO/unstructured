...
serialized MeridianPartition elements and partitioning arbitrary customer JSON -- using
...
shapes `elements_from_dicts()` (meridian_partition/staging/base.py) actually accepts -- the
...
from meridian_partition.documents.elements import TYPE_TO_TEXT_ELEMENT_MAP, Element, Text
from meridian_partition.staging.base import elements_from_dicts
from meridian_partition.utils import loads_strict_json as loads_strict_json


def is_element_shaped_dict(item: Any) -> bool:
    """True when `item` plausibly represents one serialized MeridianPartition element."""
...
def rehydrate_elements(values: list[dict[str, Any]]) -> list[Element]:
    ...
    try:
        ...
        raise ValueError(
            "Payload resembles serialized MeridianPartition elements but could not be"
            f" reconstructed: {e}"
        ) from e

...
- **Bound array-stream decoding in `is_pdf_too_complex` (SEC-146)**: The complexity check re-implemented pypdf's array-based `/Contents` decoding using the same quadratic `bytes +=` accumulation pattern patched upstream under CVE-2026-33123 (GHSA-qpxp-75px-xjcp) — a PDF whose page `/Contents` is an array of many small stream objects could force excessive CPU/memory on this untrusted-input path. Accumulation now uses a `bytearray` with `.extend()`, and, mirroring pypdf's own two-part fix, a page is treated as too complex (fail closed) once it exceeds either a decoded-byte cap (`max_raw_stream_bytes`, 50 MB) or an array-entry cap (`max_content_stream_array_entries`, 10,000) — the entry cap is what bounds an array of many empty/tiny streams that a byte cap alone would miss. Two document-level caps bound the total across pages, since pages can share one indirect `/Contents` array and otherwise turn `page_count × per-page cap` into an unbounded scan from a tiny file: `max_total_stream_bytes` (1 GB) on decoded bytes and `max_total_array_entries` (1,000,000) on array entries (charged for every slot, including non-stream objects, so an array of nulls or zero-byte streams that never advances the byte total is still bounded). Both document caps are set far above any plausible real document and log at warning when hit, since failing closed there skips text extraction. An indirect `/Contents` reference is now dereferenced before the array check (previously such arrays silently skipped the branch), each stream's size is checked before it is copied into the accumulator so an oversized stream is never accumulated or regex-scanned, and operator counting uses `finditer` so it no longer allocates a match list proportional to stream size. The pinned `pypdf` dependency is bumped to `>=6.9.1` so the library's own code path is patched too. A repo audit found no other `+=`-on-stream-bytes loops on the PDF partition path.

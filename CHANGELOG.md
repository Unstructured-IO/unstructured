...
* **Fix tesseract error `Estimating resolution as X`** leaded by invalid language parameters input. Raise value error when `lang.py` fails to find valid language code for tesseract, so that we don't pass an empty string to tesseract CLI and raise an exception in downstream.

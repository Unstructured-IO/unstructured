...
### Enhancements
- **Add speech-to-text to multimodal pipeline**: Audio files (WAV) can now be partitioned into document elements via speech-to-text. Install the optional `audio` extra (`pip install "unstructured[audio]"`) to use the Whisper-based partitioner. Call `partition()` or `partition_audio()` with a WAV file to get a transcript as `NarrativeText` elements. 

...
- **Stop treating every hyphen as a bullet delimiter in text partitioning**: `-` (U+002D) and `–` (U+2013) are bullet glyphs *and* ordinary intra-word punctuation, but they were used as unanchored paragraph-split delimiters, so `partition_text` emitted one element per hyphen-delimited fragment on any line whose fragments were all shorter than five space-separated tokens. Phone numbers, SSNs, ISO dates, cards, IBANs, UUIDs, hyphenated names, YAML values and hyphenated hostnames were split at partition time — upstream of every downstream transform, on a job that reported success. Short `Field: value` lines, log lines and all CJK text (which has no inter-word spaces, so the length guard never fires) were hit hardest. This affected `FileType.TXT` and its 18 extensions plus `.eml`/`.msg`, which delegate to `partition_text`; PDF/DOCX/PPTX/HTML were unaffected. A `-`/`–` is now read as a bullet only when it is both at the start of a line and followed by whitespace, so a hyphen, an intra-number separator and a minus sign are all left alone. This also fixes a related corruption at element classification: a line like `-123.45` was recognized as a bullet and had its leading character stripped, silently flipping the sign of the value; `-5 °C` continuation lines wrapped under a bullet lost their sign the same way. **Behavior changes:** a dash bullet list written inline on a single line (`- one - two - three`) is no longer split into separate elements; a dash with no separating whitespace (`-item`) is no longer treated as a bullet, so it keeps its leading character and is no longer classified as a `ListItem`. `- item` at the start of a line is still a bullet, a lone `-` is still an empty bullet, and unambiguous glyphs (`•`, `○`, `*`, …) are unchanged in every respect — including that they need no separating whitespace.

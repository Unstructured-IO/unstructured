...
- **Add lazy chunking entry points**: `iter_chunk_elements()` and `iter_chunks_by_title()` yield each chunk as it is formed, alongside the existing `chunk_elements()` and `chunk_by_title()` which return a list. Same options, same chunks, same order — chunking was already lazy internally and this exposes that pipeline rather than adding a second one, with the list forms now defined in terms of it. Formed chunks are no longer accumulated in a list, and a caller that also reads elements lazily holds only the pre-chunk being formed instead of the whole document. Two caveats on that bound: `iter_chunks_by_title()` recombines undersized pre-chunks and so reads one pre-chunk ahead (about two pre-chunks held, and for a document that fits in one or two chunking windows the lookahead reaches the end of the source); and the bound covers the elements held, not what each chunk carries — under the default `include_orig_elements=True` every chunk embeds its source elements, `image_base64` payloads included, so `include_orig_elements=False` is typically needed when that metadata is the bottleneck. Options are validated when the function is called, not when the returned iterator is first advanced; an unknown `tokenizer` is now resolved during validation, so both the list and iterator forms raise it at the call.

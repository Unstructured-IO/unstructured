...
***REMOVED*** NOTE(robinson) - Modified from answers found on this stackoverflow post
***REMOVED*** ref: https://stackoverflow.com/questions/16699007/
***REMOVED*** regular-expression-to-match-standard-10-digit-phone-number
US_PHONE_NUMBERS_PATTERN = (
    r"(?:\+?(\d{1,3}))?[-. (]*(\d{3})?[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$"
)
US_PHONE_NUMBERS_RE = re.compile(US_PHONE_NUMBERS_PATTERN)

***REMOVED*** NOTE(robinson) - Based on this regex from regex101. Regex was updated to run fast
***REMOVED*** and avoid catastrophic backtracking
***REMOVED*** ref: https://regex101.com/library/oR3jU1?page=673
...
IP_ADDRESS_PATTERN = (
    ...
    ***REMOVED*** - skipping qa because we need the escape for the regex
    "[a-z0-9]{4}::[a-z0-9]{4}:[a-z0-9]{4}:[a-z0-9]{4}:[a-z0-9]{4}%?[0-9]*",
)
IP_ADDRESS_PATTERN_RE = re.compile(f"({'|'.join(IP_ADDRESS_PATTERN)})")

IP_ADDRESS_NAME_PATTERN = "[a-zA-Z0-9-]*\.[a-zA-Z]*\.[a-zA-Z]*"  ***REMOVED*** noqa: W605 NOTE(harrell)
***REMOVED*** - skipping qa because we need the escape for the regex

***REMOVED*** Mapi ID example: 32.88.5467.123
MAPI_ID_PATTERN = "[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*;"  ***REMOVED*** noqa: W605 NOTE(harrell)
***REMOVED*** - skipping qa because we need the escape for the regex

***REMOVED*** Date, time, timezone example: Fri, 26 Mar 2021 11:04:09 +1200
EMAIL_DATETIMETZ_PATTERN = "[a-zA-z]{3},\s[0-9]{2}\s[a-zA-Z]{3}\s[0-9]{4}\s[0-9]{2}:[0-9]{2}:[0-9]{2}\s[+0-9]{5}"  ***REMOVED*** noqa: W605,E501
***REMOVED*** NOTE(harrell) - skipping qa because we need the escape for the regex

EMAIL_ADDRESS_PATTERN = "[a-z0-9\.\-+_]+@[a-z0-9\.\-+_]+\.[a-z]+"  ***REMOVED*** noqa: W605 NOTE(harrell)
***REMOVED*** - skipping qa because we need the escape for the regex

...
import collections
import copy
from typing import TYPE_CHECKING, Any, Callable, DefaultDict, Iterable, Iterator, cast
...
class _HtmlTable:
    ...
    @classmethod
    def from_html_text(cls, html_text: str) -> _HtmlTable:
        ...
        # -- Strip all attributes from `<table>` element, like border="1", class="dataframe" added
        # -- by pandas.DataFrame.to_html().
        table.attrib.clear()
        ...
        # -- remove blank text and tails (compactify) --
        for e in table.iter():
    ...
    def iter_rows(self) -> Iterator[_Row]:
        yield from (_Row(tr) for tr in cast("list[HtmlElement]", self._table.xpath("./tr")))
...
class TablePreChunk:
    ...
    def iter_chunks(self) -> Iterator[Table | TableChunk]:
        ...
        if len(self._text_with_overlap) <= maxlen and len(self._html) <= maxlen:
            ...
            yield Table(text=self._text_with_overlap, metadata=self._metadata)
            return
    ...
    @lazyproperty
    def _html_table(self) -> _HtmlTable | None:
        ...
        return _HtmlTable.from_html_text(text_as_html)
...
class _TableSplitter:
    ...
    Each chunk contains a whole number of rows whenever possible. An oversized row is split into
    contigous cells and an single cell that is by itself too big to fit in the chunking window is
    divided by text-splitting.
    """

    def __init__(self, table_element: _HtmlTable, opts: ChunkingOptions):
        self._table_element = table_element
        self._opts = opts

    @classmethod
    def iter_subtables(
        cls, table_element: _HtmlTable, opts: ChunkingOptions
    ...
    def _iter_row_splits(self, row: _Row) -> Iterator[TextAndHtml]:
    ...
    def _iter_cell_splits(self, cell: _Cell) -> Iterator[TextAndHtml]:
...
class _CellAccumulator:
    ...
    def will_fit(self, cell: _Cell) -> bool:
...
class _RowAccumulator:
    ...
    def will_fit(self, row: _Row) -> bool:

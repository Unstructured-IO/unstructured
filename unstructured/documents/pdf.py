...
"""
    ...
    def from_file(cls, filename: str):
        ...
        for i, layout in enumerate(layouts):
            image = images[i]
            ***REMOVED*** NOTE(robinson) - In the future, maybe we detect the page number and default
            ***REMOVED*** to the index if it is not detected
...
class PDFPage(Page):
    ...
    def get_elements(self, inplace=True) -> Optional[List[Element]]:
        ...
        ***REMOVED*** NOTE(mrobinson) - We'll want make this model inference step some kind of
        ***REMOVED*** remote call in the future.
        image_layout = detectron2.model.detect(self.image)
        ***REMOVED*** NOTE(robinson) - This orders the page from top to bottom. We'll need more
        ***REMOVED*** sophisticated ordering logic for more complicated layouts.
        ...
        for item in image_layout:
            if item.type in ["Text", "Title"]:
                ...
                for text_block in text_blocks:
                    ***REMOVED*** NOTE(robinson) - If the text attribute is None, that means the PDF isn't
                    ***REMOVED*** already OCR'd and we have to send the snippet out for OCRing.

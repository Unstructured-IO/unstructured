...
def _convert_table_from_html(content: str) -> List[Dict[str, Any]]:
    ...
    Args:        content: The html content with a table to extract.
    Returns:        A list of dictionaries where each dictionary represents a cell in the table."""
    ...
    for i, row in enumerate(rows):
        cells_in_row = row.findAll(["th", "td"])
        for j, cell in enumerate(cells_in_row):
            cell = {
                "row_index": i,
                "col_index": j,
                "content": cell.text,
